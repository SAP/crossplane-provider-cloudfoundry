...
interface. Additionally, it can be extended with =slog=
package compatible attributes—key-value pairs used for structured
logging. The =erratt.Error= type also supports wrapping Go =error=
values. When an =erratt.Error= is wrapped, its attributes are
preserved.

...
Sometimes, it is impossible to define the set of possible
/StringSlice/ parameter values upfront, in build time. The value set
may depend on a previous interactive selection of the user or on the
result of an API request.

In such cases, we can set the possible values dynamically, using the

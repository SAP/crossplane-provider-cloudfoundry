...
// +kubebuilder:validation:XValidation:rule="self.spec.managementPolicies == ['Observe'] || [has(self.spec.forProvider.orgName), has(self.spec.forProvider.orgRef), has(self.spec.forProvider.orgSelector)].filter(x, x).size() == 1",message="OrgReference validation: only one of orgName, orgRef, or orgSelector can be set"
type Space struct {

...
# Cloud Foundry Resource Exporter User Guide
...
Then, install `xpcf` by following the instructions in [Installation](#nix-users).
...
![img](/img/login.gif "Login subcommand")
...
The `login` subcommand writes the credentials to the specified configuration file. For more details about using the configuration file, refer to the [Configuration File](#configuration-file) section.
...
In addition to the [global configuration parameters](#global-configuration-parameters), the `export` subcommand also supports several subcommand-specific configuration parameters.
...
In this example, the export tool retrieves the `username`, `password`, and `apiurl` parameters from the configuration file, as they were set using the [login](#logging-in-using-username-and-password) command.

The `kind` configuration parameter is set using a CLI flag.

The *Organization* names are selected interactively.

![img](/img/cf-export-orgs-interactive.gif "Exporting interactively selected Organization resources")
...
No troubleshooting entries yet. Got a problem? [Open a GitHub issue](https://github.com/SAP/crossplane-provider-cloudfoundry/issues/new/choose) and help us build this section.

# FAQ

No FAQs yet. Got a question? [Open a GitHub issue](https://github.com/SAP/crossplane-provider-cloudfoundry/issues/new/choose) and help us build this section.

For more ways to reach us, see [👐 Support, Feedback](https://github.com/SAP/crossplane-provider-cloudfoundry#-support-feedback).

## Footnotes

<sup><a id="fn.1" class="footnum" href="#fnr.1">1</a></sup> [https://github.com/SAP/crossplane-provider-cloudfoundry/tree/main/cmd/exporter](https://github.com/SAP/crossplane-provider-cloudfoundry/tree/main/cmd/exporter)

<sup><a id="fn.2" class="footnum" href="#fnr.2">2</a></sup> [https://docs.cloudfoundry.org/cf-cli/](https://docs.cloudfoundry.org/cf-cli/)

<sup><a id="fn.3" class="footnum" href="#fnr.3">3</a></sup> [https://docs.cloudfoundry.org/cf-cli/getting-started.html#login](https://docs.cloudfoundry.org/cf-cli/getting-started.html#login)

...
- [Direct Deployment](#simple-deployment)
- [Multi-target Application (MTA)](#multi-target-application) (coming soon)

## Prerequisites: Application Runtime Quota

Before we can deploy workload to Cloud Foundry, we need to first allocate **APPLICATION_RUNTIME** quota to our BTP `Subaccount`. We can use [`Entitlement`](https://sap.github.io/crossplane-provider-docs/docs/crossplane-provider-btp/api-reference/account.btp.sap.crossplane.io/entitlement/v1alpha1) of the BTP provider for this. 
...
<summary>How do I bind non-cloudfoundry service instances (e.g. using BTP Service Operator)</summary>
    ...
    Let's assume we created [`ServiceInstance`](https://sap.github.io/crossplane-provider-docs/docs/crossplane-provider-btp/api-reference/account.btp.sap.crossplane.io/serviceinstance/v1alpha1) and [`ServiceBinding`](https://sap.github.io/crossplane-provider-docs/docs/crossplane-provider-btp/api-reference/account.btp.sap.crossplane.io/servicebinding/v1alpha1). 
...
When orchestrating `Routes`, ensure that you allocated route quota to your `Organization` using [`APPLICATION_RUNTIME Entitlement`](#application-runtime-quota) on `Subaccount` level.
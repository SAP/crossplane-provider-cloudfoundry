...
#+name: vhs_command
#+begin_src sh :result output :var vhs_command="Type `# test command` Sleep 5s" :var cli_name=config("CLI name") :var vhs_height=400
export cli_name
export vhs_name
...
#+name: vhs-code
#+begin_src sh :results output :var tmpl=vhs-code-template :var cli_name=config("CLI name") :var cmd_tmpl="${cli_name} --helppp" :var vhs_height=400
export cli_name
export vhs_height
...
** Export orgs interactively

#+name: cf-export-orgs-interactive
#+begin_example
Output vhs/cf-export-orgs-interactive.gif
Type "${cli_name} export --kind organization"  Sleep 1500ms  Enter
Sleep 2s
Down
Sleep 1s
Space
...
#+begin_src sh :results output :var tapefile=vhs-code(cmd_tmpl=cf-export-orgs-interactive,vhs_height=600)
echo $tapefile | vhs -
#+end_src
...
Enter 1
Show
Output .gif vhs/cf-export-orgs-interactive.gif
Type xpcf export --kind organization
Sleep 1500ms
Enter 1
Sleep 2s
Down 1
...
** Export serviceinstances interactively

#+name: cf-export-sis-interactive
#+begin_example
Output vhs/cf-export-sis-interactive.gif
Type "${cli_name} export --kind serviceinstance --resolve-references --output test-serviceinstance.yaml"  Sleep 1500ms  Enter
Sleep 2s
Space
Sleep 1s
Enter
...
#+begin_src sh :results output :var tapefile=vhs-code(cmd_tmpl=cf-export-sis-interactive,vhs_height=600)
echo $tapefile | vhs -
#+end_src
...
Enter 1
Show
Output .gif vhs/cf-export-sis-interactive.gif
Type xpcf export --kind serviceinstance --resolve-references --output test-serviceinstance.yaml
Sleep 1500ms
Enter 1
Sleep 2s
Space 1
...
The =login= subcommand saves the [[#apiurl][API URL]], [[#username][username]], and [[#password][password]]
configuration values to the config file.

For examples refer to [[#login-workflows][workflows chapter]].
...
:PROPERTIES:
:CUSTOM_ID: login-workflows
:END:

The [[#export][export]] subcommand accepts the [[#apiurl][API URL]], [[#username][username]], and [[#password][password]]
configuration parameters for authentication. If you prefer not to
specify these parameters for each =export= invocation, you can store
them in a [[#config-file][configuration file]].

The [[#login][login]] subcommand simplifies this process.

You can set configuration parameter values using either CLI flags or
environment variables.

Using CLI flags:

#+begin_src bash :results output :session exporter :noweb yes :var a=set-alias() :post ansi-filter(data=*this*) :eval never
<<config("CLI name")>> login --apiUrl 'https://test.cf.com' --username 'example-user' --password 'secret'
#+end_src

Using environment variables:

#+begin_src bash :results output :session exporter :noweb yes :var a=set-alias() :post ansi-filter(data=*this*) :eval never
API_URL="https://test.cf.com" USERNAME="example-user" PASSWORD="secret" <<config("CLI name")>> login
#+end_src

If any configuration value is missing, you will be prompted to enter
it:
...
#+begin_src bash :results output :session exporter :noweb yes :var a=set-alias() :post ansi-filter(data=*this*) :eval never
<<config("CLI name")>> export --use-cf-login
#+end_src
...
- The credentials and API URL of the Cloud Foundry cluster
  ([[#username][username]], [[#password][password]], [[#apiurl][apiurl]])
- The resource kind to export, which should be set to =organization=
  ([[#kind][kind]])
- A regular expression matching the organization name(s)
  to export ([[#org][org]])

In this example, the export tool retrieves the =username=, =password=, and
=apiurl= parameters from the configuration file, as they were set using the
[[#login-workflows][login]] command.

The =kind= configuration parameter is set using a CLI flag.

The /Organization/ names are selected interactively.

#+caption: Exporting interactively selected Organization resources
[[file:vhs/cf-export-orgs-interactive.gif]]

** Exporting all /Spaces/ from all /Organizations/

This example demonstrates non-interactive export of all /Spaces/ from
all /Organizations/. Configure the following:

- The credentials and API URL of the Cloud Foundry cluster
  ([[#username][username]], [[#password][password]], [[#apiurl][apiurl]])
- The resource kind to export, which should be set to =space=
  ([[#kind][kind]])
- A regular expression matching the organization name(s)
  to include ([[#org][org]])
- A regular expression matching the space name(s) to
  include ([[#space][space]])
...
The =kind=, =org=, and =space= configuration parameters are set using
CLI flags.
...
This example shows how to export all /ServiceInstance/ resources from
spaces containing the word /test/. It also demonstrates how to resolve
references so they are exported by name rather than by GUID.

Configure the following:

- The credentials and API URL of the Cloud Foundry cluster
  ([[#username][username]], [[#password][password]], [[#apiurl][apiurl]])
- The resource kind to export, set to =serviceinstance=
  ([[#kind][kind]])
- A regular expression matching the organization names
  to include ([[#org][org]])
- A regular expression matching the space names to
  include ([[#space][space]])
- A regular expression matching the service instance names to
  include ([[#serviceinstance][serviceinstance]])
- The resolve references setting ([[#resolve-references][resolve-references]])

In this example, the export tool retrieves the =username=, =password=, and
=apiurl= parameters from the configuration file, as set by the
[[#login-workflows][login]] command.

The =kind=, =org=, =space=, =serviceinstance=, and
=resolve-references= configuration parameters are set using CLI flags.

The regular expression =.*= matches all names. The regular expression
=.*test.*= matches names containing the string /test/.

The following command exports the required resources:

#+begin_src bash :results output :session exporter :noweb yes :var a=set-alias() :post ansi-filter(data=*this*)
<<config("CLI name")>> export --kind serviceinstance --org '.*' --space '.*test.*' --serviceinstance '.*' -r
#+end_src

** Exporting a /ServiceInstance/ interactively to a file

This example shows how to interactively export a specific /ServiceInstance/
resource to a file. You specify the /Organization/ and /Space/ names
interactively, then select the ServiceInstance to export.
...
The =kind=, =resolve-references=, and =output= configuration
parameters are set using CLI flags.

The =org=, =space=, and =serviceinstance= parameter values are
entered interactively.

#+caption: Exporting an interactively selected ServiceInstance resource
[[file:vhs/cf-export-sis-interactive.gif]]

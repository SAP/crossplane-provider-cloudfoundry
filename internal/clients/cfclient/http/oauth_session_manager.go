...
func (m *OAuthSessionManager) newTokenSource(ctx context.Context) error {
	...
	loginEndpoint := join(m.config.LoginEndpointURL, "/oauth/auth")
	uaaEndpoint := join(m.config.UAAEndpointURL, "/oauth/token")
...
func (m *OAuthSessionManager) userAuth(ctx context.Context, loginEndpoint, uaaEndpoint string) error {
	...
	if m.config.Origin != "" {
		...
		authConfig.Endpoint.TokenURL = format("%s?%s", authConfig.Endpoint.TokenURL, val)
	}

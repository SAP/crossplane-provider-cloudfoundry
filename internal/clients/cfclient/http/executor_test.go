...
func TestExecuteRequest(t *testing.T) {
	...
	clientProvider := NewUnauthenticatedClientProvider(httpClient)
	e := NewExecutor(clientProvider, serverURL, config.UserAgent)
	req := NewRequest(context.Background(), "GET", "/v3/organizations")
	// nolint:bodyclose
...
func TestExecuteRequestWithAuthFailure(t *testing.T) {
	...
	clientProvider := NewUnauthenticatedClientProvider(httpClient)
	e := NewExecutor(clientProvider, server.URL, config.UserAgent)
	req := NewRequest(context.Background(), "GET", "/does_not_matter")
	// nolint:bodyclose
	r, err := e.ExecuteRequest(req)
	defer func(Body io.ReadCloser) {
		_ = Body.Close()
...
`
	...
	clientProvider := NewUnauthenticatedClientProvider(httpClient)
	e := NewExecutor(clientProvider, server.URL, config.UserAgent)
	req := NewRequest(context.Background(), "POST", "/does_not_matter").WithObject(obj).WithHeader("foo", "bar")
	// nolint:bodyclose
	r, err := e.ExecuteRequest(req)
	defer func(Body io.ReadCloser) {
		_ = Body.Close()

...
import (
	...
	scv1alpha1 "github.tools.sap/cloud-orchestration/crossplane-provider-cloudfoundry/apis/v1alpha1"
	pcv1beta1 "github.tools.sap/cloud-orchestration/crossplane-provider-cloudfoundry/apis/v1beta1"
	...
	role "github.tools.sap/cloud-orchestration/crossplane-provider-cloudfoundry/internal/clients/role"
...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1alpha2.OrgRole_GroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1alpha2.OrgRole_GroupVersionKind),
		options...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		For(&v1alpha2.OrgRole{}).
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	if _, ok := mg.(*v1alpha2.OrgRole); !ok {
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	r, err := role.GetOrgRole(ctx, c.role, &cr.Spec.ForProvider)
	if err != nil {
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	if spec.Org == nil || spec.Username == nil || spec.Type == "" {

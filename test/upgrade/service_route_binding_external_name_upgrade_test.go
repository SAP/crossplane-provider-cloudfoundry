...
//
// This file (srb_external_name_upgrade_test.go) contains Test_ServiceInstance_Route_Binding_External_Name,
// which validates that ServiceRouteBinding resources maintain proper external-name formatting
...
var (
	serviceRouteBindingCustomResourceDirectories = []string{
		"./testdata/customCrs/externalNames/import",
		"./testdata/customCrs/externalNames/serviceRouteBinding",
	}
)

func Test_ServiceRouteBinding_External_Name(t *testing.T) {
	const serviceRouteBindingName = "upgrade-test-external-name-service-route-binding"

	upgradeTest := NewCustomUpgradeTest("service-route-binding-external-name-test").
		FromVersion(fromTag).
		ToVersion(toTag).
		WithResourceDirectories(serviceRouteBindingCustomResourceDirectories).
		WithCustomPreUpgradeAssessment(
			...
			func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
				...
				serviceRouteBinding := &v1alpha1.ServiceRouteBinding{}

				err = r.Get(ctx, serviceRouteBindingName, cfg.Namespace(), serviceRouteBinding)
				if err != nil {
					t.Fatalf("Failed to get ServiceRouteBinding resource: %v", err)
				}

				annotations := serviceRouteBinding.GetAnnotations()
